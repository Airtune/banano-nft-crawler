...
describe('MintBlocksCrawler', function() {
  ...
  before("run mintBlocksCrawler3mint9", async () => {
    ...
    mintBlocksCrawler3mint9 = new MintBlocksCrawler(issuer, supplyBlockHash);
    await mintBlocksCrawler3mint9.crawl(bananode).catch((error) => { throw(error) });
    ...
    infMintBlocksCrawler3mint9 = new MintBlocksCrawler(issuer, infSupplyBlockHash);
    await infMintBlocksCrawler3mint9.crawl(bananode).catch((error) => { throw(error) });
    ...
    limited2MintBlocksCrawler3mint9 = new MintBlocksCrawler(issuer, limited2SupplyBlockHash);
    await limited2MintBlocksCrawler3mint9.crawl(bananode).catch((error) => { throw(error) });
    ...
    finishedMintBlocksCrawler3mint9 = new MintBlocksCrawler(issuer, finishedSupplyBlockHash);
    await finishedMintBlocksCrawler3mint9.crawl(bananode).catch((error) => { throw(error) });
  ...
  // TODO: Add test cases for unopened accounts
  // Unopened accounts tripped me up:
  // ban_1ekkobopkdwtuqb88zeixrw3pwsm67ypsjnrzted7rndjxr3e5bhthokycc4
  // receivable: 07FE197B057E1FB1145C969E965EBA5B7F6A60E2085431E8D91917ECD9FAD8DB
  //
  // issuer: ban_3pnftpao6pbekmear374478f1ytmwz3kodcjuzf1hutcnb3gudwi9qcu8pwc
  // mint_block_hash: AA3794A0A08E337CBBB84D76A31CE667E2A420C55DAFBE35C6CA2F203DD1F263
  // Error: NanoNodeError: Account not found for ban_3cjqkkic35jmhkxuimdpneggesz4niiecqfhqrtwfkjh61yx5wm5pa3u5nm5
